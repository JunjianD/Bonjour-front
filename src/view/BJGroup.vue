    ...
    <el-aside width="260px" class="group-list-box">
      ...
      <el-scrollbar class="group-list-items">
        <div v-for="(group,index) in groupStore.groups" :key="index">
          <group-item v-show="group.remark.startsWith(searchText) || group.name.startsWith(searchText)"
                      :group="group"
          ...
          <div class="group-info">
            <div>
              <file-upload v-show="isOwner"
                ...
                <img v-if="activeGroup.headImage" :src="activeGroup.headImage" class="avatar" alt="群组头像">
                <i v-else class="icon iconfont icon-upload avatar-uploader-icon"></i>
              </file-upload>
              <head-image v-show="!isOwner"
                          class="avatar"
                          :size="200"
          ...
          <el-scrollbar style="height:200px;">
            <div class="group-member-list">
              <div v-for="(member) in groupMembers" :key="member.userId">
                <group-member v-show="!member.quit"
                              class="group-member"
                              :member="member"
              ...
              <div class="group-invite">
                <div class="invite-member-btn" title="邀请好友进群聊" @click="onInviteMember()">
                  <i class="icon iconfont icon-plus avatar-add-icon"></i>
                </div>
                <div class="invite-member-text">邀请</div>
                <add-group-member v-model="showAddGroupMember"
...
export default {
  ...
  data() {
    return {
      ...
      maxSize: 5 * 1024 * 1024,
      activeGroup: {},
      // [{userId, remark, quit,online, headImage, aliasName}]
      groupMembers: [],
  ...
  methods: {
    onCreateGroup() {
      this.$prompt('请输入群聊名称', '创建群聊', {
        ...
        cancelButtonText: '取消',
        inputPattern: /^\S+$/,
        inputErrorMessage: '请输入群聊名称，群聊名称不能包含任何空格或空白字符'
      }).then((o) => {
    ...
    onKick(member) {
      this.$confirm(`确定将成员'${member.aliasName}'移出群聊吗？`, '确认移出?', {
        ...
        this.$http({
          url: `/group-server/group/kick/${this.activeGroup.id}`,
          method: 'delete',
          params: {
    ...
    onQuit() {
      this.$confirm('退出群聊后将不再接受群里的消息，确认退出吗？', '确认退出?', {
        ...
        this.$http({
          url: `/group-server/group/quit/${this.activeGroup.id}`,
          method: 'delete'
        }).then(() => {
    ...
    onDissolve() {
      this.$confirm('确认要解散群聊吗?', '确认解散?', {
        ...
        this.$http({
          url: `/group-server/group/delete/${this.activeGroup.id}`,
          method: 'delete'
        }).then(() => {
    ...
    loadGroupMembers() {
      this.$http({
        url: `/group-server/group/members/${this.activeGroup.id}`,
        method: "get"
      }).then((members) => {
  ...
  computed: {
    ...
    isOwner() {
      return this.activeGroup.ownerId == this.$store.state.userStore.userInfo.userId;
    },
    imageAction() {
