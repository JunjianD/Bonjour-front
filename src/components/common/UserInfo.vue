  ...
  <div class="user-info-mask" @click="$emit('close')">
    <div class="user-info" :style="{left: pos.x+'px',top: pos.y+'px'}" @click.stop>
      ...
      <div class="user-btn-group">
        ...
        <el-button v-show="!isFriend && !isSelf" type="primary" @click="onAddFriend()">加为好友</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    isFriend() {
      ...
      let friend = friends.find((f) => f.id == this.user.userId);
      return friend != undefined;
    },
    isSelf(){
      return this.user.userId==this.$store.state.userStore.userInfo.userId;
    }
  }
}
</script>
