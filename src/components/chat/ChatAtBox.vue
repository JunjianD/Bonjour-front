...
export default {
  ...
  methods: {
    ...
    open(pos) {
      ...
      this.pos = pos;
      this.$nextTick(() => {
        this.init(); // ✅ 此时 DOM 已渲染，this.$refs.scrollBox 存在
      });
    },
    close() {
      this.show = false;
    ...
    scrollToActive() {
      const scrollBox = this.$refs.scrollBox;
      if (!scrollBox || !scrollBox.wrap) return;

      const { wrap } = scrollBox;
      const activeTop = this.activeIdx * 35;
      ...
      //   }
      // }
      if (activeTop > wrap.scrollTop + wrap.clientHeight) {
        wrap.scrollTop += 140;
        if (wrap.scrollTop > wrap.scrollHeight) {
  ...
  computed: {
    isOwner() {
      return this.$store.state.userStore.userInfo.userId == this.ownerId;
    }
  },
  watch: {
    searchText: {
