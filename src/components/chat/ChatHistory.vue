<template>
  <el-drawer title="聊天历史记录" size="700px" v-model="visible" direction="rtl" :before-close="onClose">
    <div class="chat-history" v-loading="loading"
         element-loading-text="拼命加载中">
      <el-scrollbar class="chat-history-scrollbar" ref="scrollbar" id="historyScrollbar" @scroll="onScroll">
        <ul>
          <li v-for="(msgInfo,idx) in messages" :key="idx">
            <chat-message-item :mode="2"
...
export default {
  ...
  methods: {
    ...
    onScroll() {
      ...
      let high = scrollWrap.scrollTop; //距离顶部的距离
    ...
    loadMessages() {
      ...
      if (this.chat.type == 'GROUP') {
        ...
        param.friendId = this.friend.userId;
      }
      this.loading = true;
      this.$http({
  ...
  computed: {
    ...
    histroyAction() {
      return `/message-server/message/${this.chat.type.toLowerCase()}/history`;
    },
    visible: {
      get() {
