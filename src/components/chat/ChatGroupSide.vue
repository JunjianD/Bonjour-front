    ...
    <el-scrollbar class="group-side-scrollbar">
      <div class="group-side-member-list">
        <div class="group-side-invite">
          ...
          <add-group-member :visible="showAddGroupMember" :groupId="String(group.id)" :members="groupMembers"
                            @reload="$emit('reload')"
      ...
      <el-form labelPosition="top" class="group-side-form" :model="localGroup">
        <el-form-item label="群聊名称">
          <el-input v-model="localGroup.name" :disabled="!editing||!isOwner" maxlength="20"></el-input>
        </el-form-item>
        <el-form-item label="群主">
          <el-input :value="ownerName" disabled></el-input>
        </el-form-item>
        <el-form-item label="群公告">
          <el-input v-model="localGroup.notice" :disabled="!editing||!isOwner" type="textarea" maxlength="1024"
                    placeholder="群主未设置"></el-input>
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="localGroup.remark" :disabled="!editing" placeholder="群聊的备注仅自己可见"
                    maxlength="20"></el-input>
        </el-form-item>
        <el-form-item label="我在本群的昵称">
          <el-input v-model="localGroup.aliasName" :disabled="!editing" placeholder="xx" maxlength="20"></el-input>
        </el-form-item>
        ...
        <div class="btn-group">
          ...
          <el-button v-show="editing" type="primary" @click="editing=!editing">取消</el-button>
...
export default {
  ...
  methods: {
    ...
    // loadGroupMembers() {
    //   this.$http({
    //     url: `/group-server/group/members/${this.group.id}`,
    //     method: "get"
    //   }).then((members) => {
    ...
    onQuit() {
      this.$confirm('退出群聊后将不再接受群里的消息，确认退出吗？', '确认退出?', {
        ...
        this.$http({
          url: `/group-server/group/quit/${this.group.id}`,
          method: 'delete'
        }).then(() => {
