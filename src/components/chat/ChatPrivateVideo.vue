<template>
  <el-dialog v-dialogDrag :title="title" top="5vh" :close-on-click-modal="false" :close-on-press-escape="false"
             v-model="visible" width="50%" height="70%" :before-close="handleClose">
    <div class="chat-video">
      <div class="chat-video-box">
        <div class="chat-video-friend" v-loading="loading" element-loading-text="等待对方接听..."
          ...
          <head-image class="friend-head-image"
                      :id="String(friend.userId)" :size="80" :name="friend.nickName"
      ...
      <div class="chat-video-controllbar">
        <div v-show="state=='CONNECTING'" title="取消呼叫" class="icon iconfont icon-phone reject"
             style="color: red;" @click="cancel()"></div>
        <div v-show="state=='CONNECTED'" title="挂断" class="icon iconfont icon-phone reject"
  ...
  methods: {
    ...
    call() {
      this.peerConnection.createOffer((offer) => {
        ...
        this.$http({
          url: `/message-server/webrtc/private/call?uid=${this.friend.userId}`,
          method: 'post',
          data: JSON.stringify(offer)
        }).then(() => {
          this.callId = this.$store.state.userStore.userInfo.userId;
          this.loading = true;
    ...
    accept(offer) {
      ...
      this.peerConnection.createAnswer((answer) => {
            ...
            this.$http({
              url: `/message-server/webrtc/private/accept?uid=${this.friend.userId}`,
              method: 'post',
              data: JSON.stringify(answer)
            }).then(() => {
              this.state = 'CONNECTED';
              this.callerId = this.friend.userId;
            })
          },
          (error) => {
            this.$message.error(error);
    ...
    handup() {
      this.$http({
        url: `/message-server/webrtc/private/handup?uid=${this.friend.userId}`,
        method: 'post'
      })
      this.close();
    ...
    cancel() {
      this.$http({
        url: `/message-server/webrtc/private/cancel?uid=${this.friend.userId}`,
        method: 'post'
      })
      this.close();
    ...
    sendFailed(reason) {
      this.$http({
        url: `/message-server/webrtc/private/failed?uid=${this.friend.userId}&reason=${reason}`,
        method: 'post'
      })
    },
    sendCandidate(candidate) {
      this.$http({
        url: `/message-server/webrtc/private/candidate?uid=${this.friend.userId}`,
        method: 'post',
    ...
    initICEServers() {
      this.$http({
        url: '/message-server/webrtc/private/iceservers',
        method: 'get'
      }).then((servers) => {
