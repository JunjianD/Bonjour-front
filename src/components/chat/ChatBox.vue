    ...
    <el-container>
      <el-header height="60px">
        ...
        <span title="群聊信息"
              ...
              class="btn-side icon iconfont icon-ellipsis"
          ...
          <el-container class="content-box">
                ...
                <ul>
                  <li v-for="(msgInfo,idx) in chat.messages" :key="idx">
                    <chat-message-item v-show="idx>=showMinIdx"
                                       :mine="msgInfo.sendId == mine.userId"
            ...
            <el-footer height="240px" class="im-chat-footer">
              <div class="chat-tool-bar">
                <div title="表情" ref="emotion"
                     @click.stop="showEmotionBox()">
                  <i class="icon iconfont icon-smile"></i>
                </div>

                <div title="发送图片">
                  <file-upload :action="'/message-server/image/upload'" :maxSize="5*1024*1024"
                ...
                <div title="发送文件">
                  <file-upload :action="'/message-server/file/upload'" :maxSize="10*1024*1024"
      ...
      <EmotionPicker ref="emoBox" @emotion="onEmotion"></EmotionPicker>
      <chat-at-box ref="atBox"
                   :ownerId="String(group.ownerId)"
                   :members="groupMembers"
                   :search-text="atSearchText"
                   @select="onAtSelect">
      </chat-at-box>
      <chat-voice v-model="showVoice" @close="closeVoiceBox" @send="onSendVoice"></chat-voice>
      <chat-history v-model="showHistory"
                    :chat="chat"
                    :friend="friend"
                    :group="group"
...
import EmotionPicker from "../common/EmotionPicker.vue";
...
export default {
  ...
  methods: {
    // 所有方法保持原样，因为 this.$http, this.$store 等依然可用
    closeRefBox() {
    ...
    showVideoBox() {
      this.$store.commit("setFriendId", this.friend);
    ...
    sendImageMessage() {
      ...
      this.$http.post("/message-server/image/upload", formData, {
        headers: {
    ...
    sendTextMessage() {
      ...
      console.log("this.chat.targetId: ", this.chat.targetId);
      // 填充对方id
      this.fillTargetId(msgInfo, this.chat.targetId);
      // 被@人员列表
      if (this.chat.type == "GROUP") {
    ...
    recallMessage(msgInfo) {
      this.$confirm('确认撤回消息?', '撤回消息', {
        ...
        let url = `/message-server/message/${this.chat.type.toLowerCase()}/recall/${msgInfo.id}`
        this.$http({
          url: url,
          method: 'delete'
        }).then(() => {
          ...
          msgInfo.status = this.$enums.MESSAGE_STATUS.WITHDRAW;
          this.$store.commit("insertMessage", msgInfo);
        }).catch((res) => {
          this.$message.error(res || "消息撤回失败");
        });
      }).catch(() => {
        this.$message.primary("取消操作");
      });
    ...
    readedMessage() {
      ...
      if (this.chat.type == "GROUP") {
        var url = `/message-server/message/group/readed?groupId=${this.chat.targetId}`
      } else {
        url = `/message-server/message/private/readed?friendId=${this.chat.targetId}`
      }
      this.$http({
        url: url,
        method: 'put'
    ...
    loadReaded(fId) {
      this.$http({
        url: `/message-server/message/private/maxReadedId?friendId=${fId}`,
        method: 'get'
      }).then((id) => {
    ...
    loadGroup(groupId) {
      this.$http({
        url: `/group-server/group/find/${groupId}`,
        method: 'get'
      }).then((group) => {
        this.group = group;
      ...
      this.$http({
        url: `/group-server/group/members/${groupId}`,
        method: 'get'
      }).then((groupMembers) => {
    ...
    loadFriend(friendId) {
      // 获取对方最新信息
      this.$http({
        url: `/user-server/user/find/${friendId}`,
        method: 'get'
      }).then((friend) => {
        this.friend = friend;
    ...
    showName(msgInfo) {
      if (this.chat.type == 'GROUP') {
        ...
        return msgInfo.sendId == this.mine.userId ? this.mine.nickName : this.chat.showName
      }
    ...
    headImage(msgInfo) {
      if (this.chat.type == 'GROUP') {
        ...
        return msgInfo.sendId == this.mine.userId ? this.mine.headImageThumb : this.chat.headImage
      }
    },
    resetEditor() {
      this.sendImageUrl = "";
  ...
  computed: {
    ...
    messageAction() {
      return `/message-server/message/${this.chat.type.toLowerCase()}/send`;
    },
    unreadCount() {
