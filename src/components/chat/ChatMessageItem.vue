  ...
  <div class="chat-msg-item">
    ...
    <div class="chat-msg-normal" v-show="msgInfo.type>=0 && msgInfo.type<10" :class="{'chat-msg-mine':mine}">
      <div class="head-image">
        <head-image :name="showName" :size="40" :url="headImage" :id="String(msgInfo.sendId)"></head-image>
      </div>
        ...
        <div class="chat-msg-bottom" @contextmenu.prevent="showRightMenu($event)">
          ...
          <div class="chat-msg-image" v-if="msgInfo.type==$enums.MESSAGE_TYPE.IMAGE">
            ...
            <span title="发送失败" v-show="loadFail" @click="onSendFail"
                  class="send-fail icon iconfont icon-warning-circle"></span>
          </div>
          ...
          <div class="chat-msg-voice" v-if="showAudio" @click="onPlayVoice()">
...
export default {
  ...
  created() {
    this.showAudio = this.msgInfo.type == this.$enums.MESSAGE_TYPE.AUDIO;
  },
  methods: {
    onSendFail() {
    ...
    menuItems() {
      ...
      if (this.msgInfo.selfSend && this.msgInfo.id > 0) {
        items.push({
          key: 'RECALL',
          name: '撤回',
          icon: 'icon iconfont icon-rollback'
        });
      }
      return items;
    },
  ...
  watch: {
    msgInfo: {
      immediate: true,
      handler(newVal) {
        this.showAudio = newVal.type == this.$enums.MESSAGE_TYPE.AUDIO;
      }
    }
  }
}
