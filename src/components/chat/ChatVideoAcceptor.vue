<template>
  <div class="chat-video-acceptor">
    <head-image :id="String(friend.userId)" :name="friend.nickName" :url="friend.headImage" :size="100"></head-image>
    <div class="acceptor-text">
      {{friend.nickName}} 请求和您进行视频通话...
    </div>
    <div class="acceptor-btn-group">
      <div class="icon iconfont icon-phone accept" @click="accpet()" title="接受"></div>
      <div class="icon iconfont icon-phone reject" @click="reject()" title="拒绝"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    accpet() {
      // let theFriend = this.friend;
      // theFriend.userId = theFriend.id;
      this.$store.commit("setFriendUserId", this.friend);
      let info = {
        friend: this.friend,
        master: false,
        offer: this.offer
    ...
    reject() {
      this.$http({
        url: `/message-server/webrtc/private/reject?uid=${this.friend.userId}`,
        method: 'post'
      })
      this.close();
    },
