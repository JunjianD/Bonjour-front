...
export default {
    ...
    mutations: {
        initChats(state, chatsData) {
            ...
            state.privateMsgMaxId = parseInt(chatsData.privateMsgMaxId) || 0;
            state.groupMsgMaxId = parseInt(chatsData.groupMsgMaxId) || 0;
            // 防止图片一直处在加载中状态
        ...
        openChat(state, chatInfo) {
            ...
            for (let idx in state.chats) {
                if (state.chats[idx].type == chatInfo.type &&
                    ...
                    state.chats[idx].showName = chatInfo.showName;
                    state.chats[idx].headImage = chatInfo.headImage;
            ...
            for (let idx in state.chats) {
                if (state.chats[idx].type == 'PRIVATE' &&
                    ...
                    state.chats[idx].messages.forEach((m) => {
                        if (m.selfSend && m.status != MESSAGE_STATUS.WITHDRAW) {
                            // pos.maxId为空表示整个会话已读
        ...
        insertMessage(state, msgInfo) {
            ...
            if (!msgInfo.selfSend && chat.type == "GROUP" && msgInfo.atUserIds &&
                ...
                if (msgInfo.atUserIds.map(String).indexOf(userId) >= 0) {
                    chat.atMe = true;
                }
            ...
            for (let idx in chat.messages) {
                ...
                let chatSendTime = chat.messages[idx].sendTime;
                let msgSendTime = msgInfo.sendTime;
                if (typeof chat.messages[idx].sendTime === 'string'){
                    chatSendTime = parseInt(chat.messages[idx].sendTime);
                }
                if (typeof msgInfo.sendTime === 'string'){
                    msgSendTime = parseInt(msgInfo.sendTime);
                }
                // 正在发送中的消息可能没有id,通过发送时间判断
                if (msgInfo.selfSend && chat.messages[idx].selfSend &&
                    chatSendTime == msgSendTime) {
                    Object.assign(chat.messages[idx], msgInfo);
                    this.commit("saveToStorage");
                    return;
                }
        ...
        saveToStorage(state) {
            // console.log("触发saveToStorage")
            // console.log(userStore.state.userInfo.userId)
            let userId = userStore.state.userInfo.userId;
